                 ...
                 dashboardPage(title="ViReMa",skin = "black",
                     dashboardHeader(tags$li(class = "dropdown",
                       ...
                       title = tags$a(href='https://www.utmb.edu/routhlab/home',
                                                            tags$img(height=50, weight=70,src='viremaLogo.svg'))),
                     ...
                     dashboardBody( #creates tabs for the sidebar
                         tabItems(
                             # First tab content
                                     ...
                                     tabBox(
                                         ...
                                         tabPanel("Sequence Explorer", 
                                                  ...
                                                  fluidRow(
                                                      ...
                                                      column(5, #generate a plot for start/stop of sequences in BED6 file
                                                             plotOutput('ssPlot',
                                                                        brush = brushOpts( id = "plot_brush"))%>% withSpinner(color="#0080ff") 
                                                      ),
                                                      column(5, #zoomed in plot for start/stop of sequences in BED6 file based on scrubbing of ssPlot
                                                             plotOutput('ssPlotZoom',click='plot_click1',
                                                                        brush = brushOpts( id = "plot_brush1"))%>% withSpinner(color="#0080ff") 
                                                      ),
                                                      box(
                                                    ...
                                                    box(
                                                      ...
                                                      tabsetPanel(
                                                        ...
                                                        tabPanel("Nucleotide Usage", #Plot of nucleotides at positions near recombination site; for brushed area in ssPlotZoom
                                                                 ...
                                                                 column(5,
                                                                        plotOutput('donorPlot')%>% withSpinner(color="#0080ff") ,
                                                                 ),
                                                                 column(5,
                                                                        plotOutput('acceptPlot')%>% withSpinner(color="#0080ff"),
                                                        ...
                                                        tabPanel("Histograms", #Plot of nucleotides at positions near recombination site; for brushed area in ssPlotZoom
                                                                 ...
                                                                 column(5,
                                                                        plotOutput('manhattanDelPlot',height = '200px') %>% withSpinner(color="#0080ff"),
                                                                 ),
                                                                 column(5,
                                                                        plotOutput('manhattanDupPlot',height = '200px') %>% withSpinner(color="#0080ff")                                                       
                                         ...
                                         tabPanel("Genomic Viewer",
                                                  ...
                                                  fluidRow(
                                                    ...
                                                    column(7,
                                                           plotOutput('circosPlot', width="600px",height="600px")%>% withSpinner(color="#0080ff") 
                                                    )
                                                  )
                                         )
                                     )
...
tabItem(tabName = "intro",
        box(width = "800px",
            ...
            tags$h3("Open an issue on our ",tags$a(href="https://github.com/routhlab/ViReMaShiny/issues", "GitHub Page"),"for troubleshooting or suggestions"),
...
server <- function(input, output, session){
    ...
    observeEvent(c(input$exampleData,input$file1),{
      ...
      if(input$exampleData == data$example_counter){
        ...
        for(i in 1:length(input$file1[,1])){
          ...
          filelist[[i]] = read.table(input$file1[[i, 'datapath']],skip=1, na.strings = "NNN", sep="\t",stringsAsFactors=FALSE, quote="") %>% mutate(File=filename)
        }
    ...
    output$manhattanDelPlot <- renderPlot({ 
      req(dataFilter() )
      mDel=dataFilter()%>% select(V2,V3,V4,V5,V6,V7) %>% filter(V2<V3) %>% mutate(id=paste0(V2,V3,V4,collapse = ",")) %>% group_by(id) %>% filter(row_number()==1) %>% ungroup()

      #This shows only parts included in the sequence table when the checkbox for tableSubset is checked
      if(input$tableSubset == TRUE & length(input$mytable_rows_all) > 0){
        
        mDel=exportDF_deb()%>% select(V2,V3,V4,V5,V6,V7) %>% filter(V2<V3) %>% mutate(id=paste0(V2,V3,V4,collapse = ",")) %>% group_by(id) %>% filter(row_number()==1) %>% ungroup()
    ...
    output$manhattanDupPlot <- renderPlot({ 
      ...
      mDup=dataFilter()%>% select(V2,V3,V4,V5,V6,V7) %>% filter(V2>V3) %>% mutate(id=paste0(V2,V3,V4,collapse = ",")) %>% group_by(id) %>% filter(row_number()==1) %>% ungroup()
      
      #This shows only parts included in the sequence table when the checkbox for tableSubset is checked
      if(input$tableSubset == TRUE & length(input$mytable_rows_all) > 0){
        
        mDup=exportDF_deb()%>% select(V2,V3,V4,V5,V6,V7) %>% filter(V2>V3) %>% mutate(id=paste0(V2,V3,V4,collapse = ",")) %>% group_by(id) %>% filter(row_number()==1) %>% ungroup()
    ...
    anno_input <- reactive({
      if (is.null(data$upload_state)) {
        ...
        anno= read.table(input$file2$datapath, na.strings = "NNN", sep="\t",stringsAsFactors=FALSE, quote="")
        return(anno)
